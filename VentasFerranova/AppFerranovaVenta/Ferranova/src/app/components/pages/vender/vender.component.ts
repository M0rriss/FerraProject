...
export class VenderComponent implements OnInit {
  ...
  if (_cantidad <= 0) {
    this._snackBar.open("La cantidad debe ser mayor que cero", "Error", {
  ...
  if (_cantidad > this.agregarProducto.stock) {
    this._snackBar.open("No hay suficiente stock para este producto", "Error", {
      horizontalPosition: "end",
      verticalPosition: "top",
      duration: 3000
    });
  ...
  registrarVenta() {
    ...
    for (const item of this.ELEMENT_DATA) {
      const producto = this.options.find(p => p.idProducto === item.idProducto);

      if (!producto || item.cantidad > Math.max(0, producto.stock)) {
        this._snackBar.open("No hay suficiente stock para algunos productos en el carrito", "Error", {
          horizontalPosition: "end",
          verticalPosition: "top",
          duration: 3000
        });
